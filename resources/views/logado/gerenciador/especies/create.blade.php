            ...
            <div class="bg-gray-50 overflow-hidden border border-gray-200 shadow-lg sm:rounded-lg pb-8">
                <div class="w-full mx-auto mt-8 sm:max-w-md px-2 py-2 bg-blue-200 shadow-md overflow-hidden sm:rounded-lg mb-2" id="form-cadastrar">
                    <div class="bg-white p-6">
                        <p class="text-center pt-2 pb-6 text-2xl font-bold">Cadastro de espécie</p>
                        <p class="mb-4 mt-8 text-xl">Digite o nome da espécie desejada, <strong>{{ Auth::user()->name }}</strong>.</p>
                        <form action="{{ route('especie.store') }}" method="post">
                            ...
                            <!-- Espécie -->
                            <div>
                                <x-text-input-float id="esNome" placeholder="Coelho" type="text" name="esNome" :value="old('esNome')" required autofocus autocomplete="esNome" />
