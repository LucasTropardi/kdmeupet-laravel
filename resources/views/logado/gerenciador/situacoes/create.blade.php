            ...
            <div class="bg-gray-50 overflow-hidden border border-gray-200 shadow-lg sm:rounded-lg pb-8">
                <div class="w-full mx-auto mt-8 sm:max-w-md px-2 py-2 bg-blue-200 shadow-md overflow-hidden sm:rounded-lg mb-2" id="form-cadastrar">
                    <div class="bg-white p-6">
                        <p class="text-center pt-2 pb-6 text-2xl font-bold">Cadastro de situação</p>
                        <p class="mb-4 mt-8 text-xl text-left">Digite a situação desejada, <strong>{{ Auth::user()->name }}</strong>.</p>
                        <form action="{{ route('situacao.store') }}" method="post">
                            ...
                            <!-- Situação -->
                            <div class="p-2">
                                <x-text-input-float id="situacao" placeholder="Situação" class="block mt-1 w-full" type="text" name="situacao" :value="old('situacao')" required autofocus autocomplete="situacao" />
